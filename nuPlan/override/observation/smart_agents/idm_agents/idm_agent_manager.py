...
from override.observation.smart_agents.idm_agents.idm_states import IDMLeadAgentState
...
from nuplan.planning.simulation.observation.smart_agents.idm_agents.idm_agent_manager import (
    IDMAgentManager as IDMAgentManagerParent,
)

UniqueIDMAgents = Dict[str, IDMAgent]


class IDMAgentManager(IDMAgentManagerParent):
    def __init__(self, agents: UniqueIDMAgents, agent_occupancy: OccupancyMap):
        super().__init__(agents, agent_occupancy)
        # self.agents: UniqueIDMAgents = agents
        # self.agent_occupancy = agent_occupancy

    def propagate_agents(
        self, ego_state: EgoState, tspan: float, iteration: int
    ) -> None:
        """
        ...
        for agent_token, agent in self.agents.items():
            ...
            # print(agent._state.velocity, agent._state.progress)
    ...
    def get_active_agents(
        self, iteration: int, num_samples: int, sampling_time: float
    ) -> Detections:
        """
        ...
        for (
            agent_token,
            agent,
        ) in self.agents.items():  # for agent in self.agents.values():
            if agent.is_active(iteration):
                box = agent.get_box_with_planned_trajectory(
                    num_samples, sampling_time, agent_token
                )
