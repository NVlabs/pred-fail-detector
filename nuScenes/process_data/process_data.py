...
def process_scene(token_samples, env, data_path, data_class):
    ...
    scene = Scene(timesteps=max_timesteps + 1, dt=env.dt, name=sample_token, aug_func=augment,
                  x_min=x_min, y_min=y_min, x_max=x_max, y_max=y_max, map_name=map_name)
...
def process_data(data_path, version, output_path, num_workers):
    ...
    for data_class in ['train_val', 'val', 'train']: # ['mini_train', 'mini_val']: 
        ...
        samples_list = list()
        for instance_sample_token in tqdm(instance_sample_tokens, desc=f'Preprocessing {data_class} Samples'):
            ...
            for sample in samples:
                ...
                ego_data[sample_data_token] = nusc.get('ego_pose', sample_data['ego_pose_token'])
        ...
        if len(scenes) > 0:
            data_dict_path = os.path.join(output_path, 'nuScenes_' + data_class + '.pkl')
